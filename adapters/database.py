...
from ..domain.product import Product, Chopp, Returnable, IsotonicWater, DisposableProduct
...
def create_product(item_dto) -> Product:
    if getattr(item_dto, 'Barril', False):#Keg
        return Chopp()
    if getattr(item_dto, 'Retornável', False):#Returnable
        return Returnable()
    if getattr(item_dto, 'Água/Isotônico', False):
