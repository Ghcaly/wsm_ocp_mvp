...
class DomainOperations:
    ...
    def join_grouped_spaces(self, context, items, grouped_spaces):
        """Port of C# JoinGroupedSpaces: if there are group codes with no mounted space
        in the grouped_spaces then delegate to join_spaces."""
        if not grouped_spaces:
            return

        # collect group codes from items
        group_codes = {getattr(it.Product.PackingGroup, "GroupCode") for it in items}

        # flatten grouped_spaces
        flattened = [ms for grp in grouped_spaces for ms in grp]

        def has_space_and_not_blocked(ms):
            return getattr(ms, "Space", None) is not None and not getattr(ms, "Blocked", False)

        def with_group_code_and_not_chopp(ms, code):
            for container in getattr(ms, "Containers", []):
                for mp in getattr(container, "Products", []):
                    pg = getattr(mp.Product, "PackingGroup", None)
                    if not pg:
                        continue
                    pg_code = getattr(pg, "GroupCode", None)
                    # determine if product is chopp
                    is_chopp = getattr(mp.Product, "IsChopp", None)
                    try:
                        chopp_flag = is_chopp() if callable(is_chopp) else bool(is_chopp)
                    except Exception:
                        chopp_flag = False
                    if pg_code == code and not chopp_flag:
                        return True
            return False

        groups_without_mounted_spaces = [
            code for code in group_codes
            if not any(has_space_and_not_blocked(ms) and with_group_code_and_not_chopp(ms, code) for ms in flattened)
        ]

        if not groups_without_mounted_spaces:
            return

        # delegate to join_spaces (python equivalent of JoinSpaces)
    ...
    def get_mounted_products_with_occupation(self, base_space, space1, space2, calculate_additional_occupation):
        ...
        for container in base_space.Containers:
            for mp in container.Products:
                occ1 = self.FactorConverter().occupation(mp.Amount, mp.Product.GetFactor(space1.Space.Size), mp.Product.PalletSetting, mp.Item, calculate_additional_occupation)
                occ2 = self.FactorConverter().occupation(mp.Amount, mp.Product.GetFactor(space2.Space.Size), mp.Product.PalletSetting, mp.Item, calculate_additional_occupation)
                results.append({
                    "mounted_product": mp,
    ...
    def change_product(self, context, space1, space2):
        ...
        mounted_products_space1 = self.get_mounted_products_with_occupation(space1, space1, space2, context.Settings.OccupationAdjustmentToPreventExcessHeight)
        mounted_products_space2 = self.get_mounted_products_with_occupation(space2, space1, space2, context.Settings.OccupationAdjustmentToPreventExcessHeight)
    ...
    def add_splitted_product_on_space(self, context, space, item):
        ...
        try:
            ...
            quantity_to_remaining_space = int(math.floor(float(qty_decimal)))
        except Exception:
        ...
        try:
            if hasattr(context, 'add_product'):
                context.add_product(space, item, quantity_to_remaining_space, first_layer, quantity_of_layer, occupation)
            else:
    ...
    def change_product_full_space(self, context, mounted_space_to_add, mounted_space_to_remove):
        """Attempt to move all products from mounted_space_to_remove into mounted_space_to_add.

        Faithful, direct-port of the C# ChangeProductFullSpace behaviour but simplified for
        the Python domain: uses the project's FactorConverter to compute occupations,
        checks if the products to remove fit into the remaining occupation of the
        destination mounted space, verifies can_add for each mounted product, and
        delegates the actual switch to Context.switch_products.

        This implementation calls attributes/methods directly (no defensive guards)
        as requested.
        """
        ...
        # compute occupations of products on the mounted space to remove using dest size
        products_to_remove = mounted_space_to_remove.get_first_pallet().get_products()
        quantity_products_on_pallet_to_remove = []
        for x in products_to_remove:
            occ = fc.occupation(getattr(x, 'amount', x.Amount if hasattr(x, 'Amount') else x.amount),
                                x.product.get_factor(space_size_to_add),
                                x.product.PalletSetting,
                                getattr(x, 'item', None),
                                context.get_setting('OccupationAdjustmentToPreventExcessHeight', False))
            quantity_products_on_pallet_to_remove.append({'product': x, 'occupation': occ})

        total_occ_to_remove = sum([float(q['occupation']) for q in quantity_products_on_pallet_to_remove])

        # check if fits on destination remaining occupation
        remaining = float(getattr(mounted_space_to_add, 'occupation_remaining', None) or (float(mounted_space_to_add.space.size) - float(getattr(mounted_space_to_add, 'occupation', 0))))
        if total_occ_to_remove > remaining:
            return False

        # compute new occupation summing all products as they'd occupy on the dest size
        all_products = list(mounted_space_to_add.get_first_pallet().get_products()) + list(products_to_remove)
        new_occupation = 0.0
        for x in all_products:
            occ = fc.occupation(getattr(x, 'amount', x.Amount if hasattr(x, 'Amount') else x.amount),
                                ...
                                context.get_setting('OccupationAdjustmentToPreventExcessHeight', False))
            try:
                new_occupation += float(occ)
            except Exception:
                new_occupation += float(getattr(occ, 'value', 0) or 0)

        # verify can_add for each mounted product on the remove side
        for q in quantity_products_on_pallet_to_remove:
            mp = q['product']
            qty = int(getattr(mp, 'amount', getattr(mp, 'Amount', 0)))
            if not self.can_add(context, mounted_space_to_add, getattr(mp, 'item', None), qty):
                return False

        # perform the switch on context
        try:
            context.switch_products(mounted_space_to_add, mounted_space_to_remove, new_occupation)
            print(f"DomainOperations: switched products from {mounted_space_to_remove.space.number} to {mounted_space_to_add.space.number}, new occupation {new_occupation}")
            return True
        except Exception:
            return False
    ...
    def switch_spaces(self, context:object, spaceDto1:object, spaceDto2:object, auxiliary_space:object ):
        """SwitchSpaces(context, SpaceWithMountedSpaceDto, SpaceWithMountedSpaceDto)
        ...
        if getattr(spaceDto1, 'Space', None) is not None:
            try:
                ...
                try:
                    spaceDto1.Space = auxiliary_space
                except Exception:
                    setattr(spaceDto1.MountedSpace, 'Space', spaceDto2.Space)
        else:
            try:
                if hasattr(context, 'AddSpace'):
                    context.AddSpace(auxiliary_space)
                if hasattr(context, 'RemoveSpace'):
                    context.RemoveSpace(auxiliary_space)
            except Exception:
                pass
