from ocp_score_ia.domain.space_list import SpaceList
...
class PalletEqualizationRule(BaseRule):
    ...
    def _equalize_pallets(self, context: Context, pallets_to_equalize: List[PalletEqualizeDto]) -> None:
        ...
        for target_space in empty_spaces:
            for pallet in [p for p in pallets_to_equalize if not p.IsEqualized]:
                ...
                if not self.can_add_products_in_space(pallet.ProductsToMove, target_space, context.get_setting('OccupationAdjustmentToPreventExcessHeight', False)):
                    continue
    ...
    def can_add_products_in_space(self, products_to_move: list, space: Any, occupation_adjustment: bool) -> bool:
        """
        Verifica se os produtos cabem no espaço, truncando a ocupação de cada produto para 2 casas decimais.
        Equivalente ao método C# CanAddProductsInSpace.
        """
        total = sum(
            (self._factor_converter.Occupation(p, space.Size, p.Item, occupation_adjustment) * 100) / 100
    ...
    def _select_pallets_with_multiple_groups(self, context: Context, out_list: List[PalletEqualizeDto], min_percent: int) -> None:
        mounted_spaces = context.MountedSpaces.NotBulk().NotContainProductComplex().WithMultipleGroups().Matching(lambda x: x.OccupiedPercentage >= context.get_setting('PercentOccupationMinByDivision'))
    ...
    def _select_pallets_with_multiple_packages(self, context: Context, out_list: List[PalletEqualizeDto], min_percent: int) -> None:
        mounted_spaces = context.MountedSpaces.NotBulk().NotContainProductComplex().WithSameGroupAndMultiplePackages().Matching(lambda x: x.OccupiedPercentage >= context.get_setting('PercentOccupationMinByDivision'))
    ...
    def _select_pallets_with_multiple_items(self, context: Context, out_list: List[PalletEqualizeDto], min_percent: int) -> None:
        mounted_spaces = context.MountedSpaces.NotBulk().NotContainProductComplex().WithSameGroupAndPackageAndMultipleItems().Matching(lambda x: x.OccupiedPercentage >= context.get_setting('PercentOccupationMinByDivision'))
        for ms in mounted_spaces:
            products = ms.GetProducts()
            if len(products) <= 1:
                continue
