...
def create_product(item_dto, is_marketplace=False) -> Product:
    """
    ...
    """
    # Se for marketplace, retorna Package (ContainerType.PACKAGE)
    if is_marketplace:
        print(f"[MARKETPLACE] Produto {getattr(item_dto, 'Código', 'Unknown')} marcado como PACKAGE (marketplace)")
        return Package()
...
def fill_item_from_row(item, combined_groups, support_point, row):
    ...
    # Verifica se o produto é marketplace
    marketplace_skus = load_marketplace_skus()
    is_marketplace = str(item.Code) in marketplace_skus
    
    item.Product = create_product(row, is_marketplace=is_marketplace)
...
def enrich_items(items_list, combined_groups, support_point, df):
    ...
    # print("[INFO] Iniciando enriquecimento dos items com dados do DataFrame")
    ...
    for item in items_list:
        ...
        if code not in df.index:
            # print(f"[AVISO] Código {code} não existe no DataFrame")
        ...
        if isinstance(row_data, pd.DataFrame):
            # print(f"[AVISO] Código {code} tem múltiplas entradas. Aplicando prioridade para unidades SP.")

            # ajuste aqui o nome da coluna que identifica o CD
            coluna_unidade = "Armazém"

            # Filtrar apenas as linhas pertencentes a unidades SP
            # sp_rows = row_data[row_data[coluna_unidade].isin(UNIDADES_SP)].reset_index(drop=True)
            # print(len(sp_rows))
            # if len(sp_rows) > 1:
            #     print(f"[INFO] Entrada mais de uma unidade SP para {code}. Selecionando a com mais colunas preenchidas")
            #     print(type(sp_rows))
            #     sp_rows = sp_rows[sp_rows]
            #     # Conta quantas colunas estão preenchidas
            #     filled_counts = sp_rows.notna().sum(axis=1)

            #     # Pega o índice da linha com mais preenchimento
            #     best_index = filled_counts.idxmax()
            #     row = sp_rows.iloc[best_index]
                # row = sp_rows.iloc[0]
            if len(row_data) > 1:
                # print(f"[INFO] Entrada mais de uma unidade SP para {code}. Selecionando a primeira")
                # print(row_data)
                # row_data = row_data[row_data['Código Unb'].isna()]
                row_data = row_data[(row_data['Código Unb'] == 'None') | (row_data['Código Unb'] == 'nan')]
                row = row_data.iloc[0]
            elif len(row_data) == 1:
                # print(f"[INFO] Entrada de unidade SP encontrada para {code}.")
                row = row_data.iloc[0]
            else:
                # print(f"[INFO] Nenhuma unidade SP encontrada para {code}. Usando a primeira entrada.")
                row = row_data.iloc[0]

        else:
            # print("Item carregado com sucesso:", code)
            row = row_data
...
# def enrich_items(items_list, support_point, df):
#     """
#     Enriquece os items com dados do DataFrame.
#     Assume que pode haver códigos duplicados no DataFrame.
#     """
#     print("[INFO] Iniciando enriquecimento dos items com dados do DataFrame")
...
#         # Se retornar DataFrame (múltiplas linhas), pega a primeira
#         if isinstance(row_data, pd.DataFrame):
#             print(f"[AVISO] Código {code} tem múltiplas entradas. Usando a primeira.")
#             row = row_data.iloc[0]
#         else:
#             print("Item carregado com sucesso:", code)
#             row = row_data
        
#         enriched.append(fill_item_from_row(item, support_point, row))
